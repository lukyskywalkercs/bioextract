...
REGLA 12 — CLASIFICACIÓN METODOLÓGICA DE SEGUNDO NIVEL:
Además del tipo_estudio, detecta el diseño exacto buscando estos clusters
de palabras en el abstract:
- "retrospectivo":      archive, FFPE, historical, previously collected,
                        review of records, formalin-fixed, paraffin-embedded,
                        stored samples
- "prospectivo":        enrolled, followed for, prospective, longitudinal,
                        will be, ongoing
- "corte_transversal":  at the time of, prevalence, cross-sectional,
                        point-in-time, snapshot
- "caso_control":       matched, comparison group, healthy controls,
                        reference group, cases and controls
- "desconocido":        si ninguno de los anteriores aplica
Asigna este valor al campo "diseno_metodologico" en metadata y
resumen_ejecutivo.
...
{{
  ...
  "metadata": {{
    ...
    "diseno_metodologico": "retrospectivo|prospectivo|corte_transversal|caso_control|desconocido"
  }},
  "resumen_ejecutivo": {{
    "tipo_estudio": "mismo valor que metadata.tipo_estudio",
    "diseno_metodologico": "mismo valor que metadata.diseno_metodologico"
  }}
}}
...
def _check_survival_triggers(payload, abstract_text):
    SURVIVAL_TRIGGERS = {
        "prognosis", "prognostic", "outcome", "endpoint",
        "follow-up", "followup", "predictor", "mortality",
        "long-term", "survival", "pfs", "os",
        "time-to-event", "pronóstico"
    }
    texto = str(abstract_text).lower()
    tiene_trigger = any(t in texto for t in SURVIVAL_TRIGGERS)
    if not tiene_trigger:
        return payload
    entidades = payload.get("entidades_de_riesgo", [])
    tiene_hr_ci = any(
        isinstance(e.get("metricas"), dict)
        and (
            e["metricas"].get("HR") is not None
            or e["metricas"].get("ci_lower") is not None
            or e["metricas"].get("ci_upper") is not None
        )
        for e in entidades
    )
    if not tiene_hr_ci:
...
def _check_minimum_metrics(payload, abstract_text):
    tipo = (
        payload.get("metadata", {}).get("tipo_estudio")
        or payload.get("resumen_ejecutivo", {}).get("tipo_estudio")
        or ""
    )
    entidades = payload.get("entidades_de_riesgo", [])

    tiene_pvalue = any(
        isinstance(e.get("metricas"), dict) and e["metricas"].get("p_value") is not None
        for e in entidades
    )
    tiene_hr = any(
        isinstance(e.get("metricas"), dict) and e["metricas"].get("HR") is not None
        for e in entidades
    )
    tiene_or = any(
        isinstance(e.get("metricas"), dict) and e["metricas"].get("OR") is not None
        for e in entidades
    )
    tiene_ci = any(
        isinstance(e.get("metricas"), dict)
        and (
            e["metricas"].get("ci_lower") is not None
            or e["metricas"].get("ci_upper") is not None
        )
        for e in entidades
    )
    ...
    if mode == "Individual":
        ...
        for idx, (row_idx, abstract) in enumerate(abstracts_series.items(), start=1):
            ...
            try:
                ...
                payload = _recalcular_entidades_con_metricas(payload)
                payload = _check_survival_triggers(payload, abstract.strip())
                payload = _check_minimum_metrics(payload, abstract.strip())
